...
BEGIN
  ...
  IF _leader.invite_email IS NULL OR btrim(_leader.invite_email) = '' THEN RAISE EXCEPTION 'Leader email is required'; END IF;
  IF EXISTS (SELECT 1 FROM public.leaders l WHERE l.campaign_id = _leader.campaign_id AND l.id <> _leader.id AND lower(l.invite_email) = lower(_leader.invite_email) AND l.user_id IS NULL) THEN
  ...
  UPDATE public.user_invitations SET expires_at = now() WHERE lower(email) = lower(_leader.invite_email) AND campaign_id = _leader.campaign_id AND accepted_at IS NULL AND expires_at > now();
